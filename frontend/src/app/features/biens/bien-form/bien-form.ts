...
import { Photo, StatutBien } from '../../../core/models/bien.models';

@Component({
  selector: 'app-bien-form',
  imports: [ReactiveFormsModule],
  templateUrl: './bien-form.html',
  styleUrl: './bien-form.css',
})
export class BienForm {
  ...
  constructor() {
    // Si l'URL contient un id (/biens/:id/modifier) -> mode édition : on pré-remplit.
  ...
  onSubmit(): void {
    ...
    if (this.modeEdition() && this.bienId !== null) {
      // On conserve le statut actuel (publier est une action séparée).
  ...
  // Recharge UNIQUEMENT la liste des photos (sans réécraser le formulaire en cours d'édition).
